...
#         if filename.split('.')[1] == 'tif' and filename.split('.')[0].strip().lower() in analyse:
#             full_filename = dirpath + '/' + filename
#             new_filename = filename.split('.')[0].strip().lower() + "/" + str(INDICE) + '.png'
#             cell_mask_filename = filename.split('.')[0].strip().lower() + "/cell_mask/" + str(INDICE) + '.png'
#             nucleus_mask_filename = filename.split('.')[0].strip().lower() + "/nucleus_mask/" + str(INDICE) + '.png'
#             mtoc_input = filename.split('.')[0].strip().lower() + "/mtoc/" + str(INDICE) + '.png'
#             mtoc_target = filename.split('.')[0].strip().lower() + "/mtoc/target/" + str(INDICE) + '.png'
#             print(full_filename)
#             INDICE += 1
#             try:
#                 im = io.imread(full_filename)
#                 max_proj = get_max_projection(im).astype(np.float32)
#                 cell_mask = get_mask(full_filename, 'cell_mask')
#                 nucleus_mask = get_mask(full_filename, 'nucleus_mask')
#                 mtoc_data, mtoc_label = create_mtoc_data(max_proj, cell_mask, full_filename)
#                 imageio.imwrite(save_path + '/' + new_filename, max_proj)
#                 imageio.imwrite(save_path + '/' + cell_mask_filename, cell_mask)
#                 imageio.imwrite(save_path + '/' + nucleus_mask_filename, nucleus_mask)
...
if __name__ == "__main__":
    ...
    #
    # for directory, filenames in new_list:
    #     for file in filenames:
