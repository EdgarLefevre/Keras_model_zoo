...
def conv_block(inputs, filters, w_decay, strides=1, block_name=""):
    ...
    x = layers.ReLU()(x)
    x = layers.Conv2D(filters, 3, padding="same", strides=strides, kernel_regularizer=regularizers.l2(w_decay))(x)
    return x


def shortcut(inputs, filters, strides, block_name=""):
    x = layers.BatchNormalization()(inputs)
    x = layers.ReLU()(x)
    x = layers.Conv2D(filters, 1, strides=strides, name=block_name+"_shortcut")(x)
    return x
...
def bottleneck(inputs, filters, strides, w_decay, block_name):
    x = layers.BatchNormalization()(inputs)
    x = layers.ReLU()(x)
    x = layers.Conv2D(filters // 4, 1, strides=strides, kernel_regularizer=regularizers.l2(w_decay),
                      kernel_initializer=tf.keras.initializers.glorot_normal(), name=block_name+"_conv1")(x)
    # x = layers.Dropout(0.3)(x)
    x = layers.BatchNormalization()(x)
    x = layers.ReLU()(x)
    x = layers.Conv2D(filters // 4, 3, padding="same", kernel_regularizer=regularizers.l2(w_decay),
                      kernel_initializer=tf.keras.initializers.glorot_normal(), name=block_name+"_conv2")(x)
    x = layers.Dropout(0.34)(x)
    x = layers.BatchNormalization()(x)
    x = layers.ReLU()(x)
    x = layers.Conv2D(filters, 1, kernel_regularizer=regularizers.l2(w_decay),
                      kernel_initializer=tf.keras.initializers.glorot_normal(), name=block_name+"_conv3")(x)
    # x = layers.Dropout(0.3)(x)
    return x


def resnet_block(filters, inputs, strides=1, w_decay=0.0001, block_name=""):
    x = conv_block(inputs, filters, w_decay, strides=strides, block_name=block_name+"_conv1")
...
def resnet_bottleneck_block(filters, inputs, block_name, strides=1, w_decay=0.0001):
...
def resnet34(input_shape, lr, nb_classes=2, base_filter=16):
    ...
    b1_c1 = layers.Conv2D(filters=base_filter, kernel_size=7, strides=2, padding='same', name="block1_conv1")(inputs)
    ...
    pool = layers.GlobalAveragePooling2D()(b5_c3)
    ...
    model.compile(loss=keras.losses.BinaryCrossentropy(), optimizer=keras.optimizers.Adam(lr=lr),
                  metrics=['accuracy'])
    return model
...
def resnet18(input_shape, lr, nb_classes=2, base_filter=16):
    ...
    b1_c1 = layers.Conv2D(filters=base_filter, kernel_size=7, strides=2, padding='same')(inputs)
    b1_bn = layers.BatchNormalization()(b1_c1)
    b1_relu = layers.ReLU()(b1_bn)
    b1_mp = layers.MaxPooling2D(pool_size=3, strides=2)(b1_relu)
...
def resnet50(input_shape, lr, nb_classes=2, base_filter=8):
    inputs = layers.Input(input_shape)

    b1_c1 = layers.Conv2D(filters=base_filter, kernel_size=7, strides=2, padding='same',
                          kernel_initializer=tf.keras.initializers.glorot_normal(), name="block1_conv1")(inputs)
    ...
    # b3_c1 = resnet_bottleneck_block(base_filter * 2, b2_c1, "block3_1", strides=2)
    ...
    # b4_c1 = resnet_bottleneck_block(base_filter * 4, b3_c2, "block4_1", strides=2)
    ...
    fc1 = layers.Dense(base_filter, activation='relu', kernel_constraint=tf.keras.constraints.min_max_norm())(pool)
    dp1 = layers.Dropout(0.5)(fc1)
    # fc2 = layers.Dense(base_filter * 2, activation='relu', kernel_constraint=tf.keras.constraints.min_max_norm())(dp1)
    # dp2 = layers.Dropout(0.5)(fc2)
    fc3 = layers.Dense(nb_classes, activation='softmax')(dp1)
