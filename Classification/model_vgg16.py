...
def block_conv(inputs, kernel_size, filters, nb_conv=2, drop=0.3):
    x = layers.Conv2D(filters, (kernel_size, kernel_size), kernel_initializer='he_normal', padding='same')(inputs)
    x = layers.Dropout(drop)(x)
    x = layers.LeakyReLU(alpha=0.1)(x)
    for i in range(nb_conv - 1):
        x = layers.Conv2D(filters, (kernel_size, kernel_size), kernel_initializer='he_normal', padding='same')(x)
